                ...
                <table>
                    ...
                    <tbody>
                            @foreach ($horaires as $horaire)
                                <tr>
                                    <td><img src="{{ $horaire->groupe->image_url }}" width="300" height="200" alt="groupe musique"></td>
                                        <td class="nom">{{ $horaire->groupe->nom }}</td>
                                        <td class="ville">{{ $horaire->groupe->ville }}</td>
                                        <td class="date-heure">
                                            {{ ucfirst($horaire->date->locale('fr_FR')->isoFormat('dddd D MMMM YYYY')) }} - {{ $horaire->heure }}
                                        </td>
                                </tr>
                            @endforeach
                    </tbody>
